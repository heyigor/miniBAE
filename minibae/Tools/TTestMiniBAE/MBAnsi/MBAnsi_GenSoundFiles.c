...
**	3/7/2000	Fixed compiler warnings
**	3/13/2000	msd: added GM_ReadRawAudioIntoMemoryFromMemory() and integrated DSP messaging
**					 for loading sounds.
**
*/
/*****************************************************************************/
...
static long IFF_PutChunk(X_IFF *pIFF, long block, long unsigned size, XPTR p)
{
	...
	{
		long tmp = 0;
		IFF_WriteBlock(pIFF, (XPTR)&tmp, sizeof(long));
		IFF_WriteBlock(pIFF, (XPTR)&tmp, sizeof(long));
	}
					...
					else	// if (decodeData) // for now
					{
						...
						if (wave->theWaveform)
						{
							...
							err = PV_ReadSunAUFile(encoding, file,
													wave->theWaveform, wave->waveSize, NULL);
							if (err == 0)
							{
...
GM_Waveform		*dspWaveform;
	...
	err = GM_TransferWaveform(waveform, &dspWaveform);
	if (err == NO_ERR)
	{
		XDisposePtr(waveform->theWaveform);
		XDisposePtr(waveform);
		waveform = dspWaveform;
	}
#endif
	...
	if (sampleData)
	{
		...
		if (pWave)
		{
			...
			if (copySampleData != NULL)
			{
				...
				pWave->bitSize = bitSize;
				pWave->channels = channels;
...
#if BAE_MCU
	err = GM_TransferWaveform(pWave, &pWaveDSP);
	if (err == NO_ERR)
	{
		XDisposePtr(pWave->theWaveform);
		XDisposePtr(pWave);
		pWave = pWaveDSP;
	}
#endif
...
// ***************************** OutputToFile ***************
// **********************************************************
// The next two methods added from BAE 11/28/00 tom
// GM_FinalizeFileHeader and GM_WriteAudioBufferToFile
// used by BAEMixer *OutputToFile methods
// **********************************************************
// **********************************************************
...
//
// Currently only works on WAVE files.
//
			...
			case FILE_AIFF_TYPE:
			{
				...
				if (XGetLong(&chunk) == X_FORM)
				{
					...
					if (XGetLong(&chunk) == X_AIFF)
					{
						...
						tmp = fileSize - XFileGetPosition(file) - sizeof(chunk) - 8; // 8 unused bytes at beginning of SSND chunk
